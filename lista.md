...
Função MultiplicaçãoDasMatrizes(matrizA, matrizB):
    ...
    Se tamanho(matrizA[0]) ≠ tamanho(matrizB) então:
        Escrever("As matrizes não podem ser multiplicadas. Elas têm dimensões incompatíveis.")
        Retornar nulo
    Senão:
...
matrizProduto <- MultiplicaçãoDasMatrizes(matrizA, matrizB)
Se matrizProduto ≠ nulo então:
